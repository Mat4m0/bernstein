...
- **Typ:** `(ctx: HmrContext) => Array<ModuleNode> | void | Promise<Array<ModuleNode> | void>`
  ...
  - Ein leeres Array zurückgeben und die vollständige benutzerdefinierte HMR-Verarbeitung durch Senden benutzerdefinierter Ereignisse an den Client durchführen:
    ...
    Der Client-Code sollte den entsprechenden Handler mit der [HMR-API](api-hmr__de%20-%20🇩🇪.md) registrieren (dies kann durch denselben `transform`-Hook des Plugins injiziert werden):
